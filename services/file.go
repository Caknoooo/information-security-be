...
type (
	FileService interface {
		...
		GetLastSubmittedFilesByUserId(ctx context.Context, userId string) ([]dto.UploadFileResponse, error)
...
func (s *fileService) GetLastSubmittedFilesByUserId(ctx context.Context, userId string) ([]dto.UploadFileResponse, error) {
	result, err := s.fileRepo.GetLastSubmittedFilesByUserId(ctx ,nil, userId, []string{constants.ENUM_FILE_TYPE_FILE, constants.ENUM_FILE_TYPE_IMAGE, constants.ENUM_FILE_TYPE_VIDEO})
	if err != nil {
		return []dto.UploadFileResponse{}, err
	}
